...
class DynamicConsoleTable(object):
    ...
    def _format_arg(self, arg, properties, args):
        if arg == '':
        ...
        color_fn = properties.get('color_fn', None)
        if properties.get('align', None) == 'left':
            s = s + ' ' * space
        elif properties.get('align', None) == 'center':
            s = ' ' * int(math.ceil(space / 2.0)) + s + ' ' * int(space / 2.0)
        else:
            s = ' ' * space + s
        superprefix = properties.get('superprefix', None)
        if superprefix:
            s = superprefix + s[len(superprefix):]
        if color_fn:
            s = chalk.Chalk(color_fn(args))(s) + chalk.RESET
        elif color:
            s = chalk.Chalk(color)(s) + chalk.RESET
        return s
    ...
    def update(self, *args):
        ...
        for i in range(len(self.layout)):
            formatted = self._format_arg(args[i], self.layout[i], args)
            row += ' ' + formatted + ' |'
